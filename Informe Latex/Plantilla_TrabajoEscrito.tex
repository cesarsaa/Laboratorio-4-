...
~\\ Dado que los salarios se encuentran distribuidos normalmente con media ${\mu}$ y varianza ${\sigma^2}$ $Salarios ~ N(\mu = 13.200, \sigma^2 = 1742400)$ Por lo cual se esta acusando a una compañia de pagar por debajo del est\'{a}ndar nacional. 
~\\ Para contrastar esta situaci\'{o}n se ha tomado una muestra de 40 trabajadores sobre los cuales se observa un pago promedio de $\$12000/hora.$ 
...
~\\ Estadístico de Prueba $ Z_{p} = \frac{\bar x - \mu}{\sigma^2/\sqrt{n}}$

~\\ $ Z_{p} = \frac{12000 - 13200}{1320/\sqrt{40}} $
~\\ $ Z_{p} = -5.7495$

~\\  La region de rechazo asignando una confianza del 95\% sera RC = {$\left \{ Z_{p} < Z_{\alpha} \right \} \to  -5.7495 < -1.64$}

~\\ \begin{figure}[!h]
    \begin{center}
        \includegraphics[width=10cm]{Figuras/punto1.png}
        \caption{Región de rechazo para $H_{o} : \mu = 13.200$ y $H_{1} : \mu < 13.200$}
        \label{fig:Densidad}
    \end{center}
\end{figure}
...
\pagebreak
~\\ \textbf{Conclusión:}
~\\ Por lo cual, con una significancia del $5\%$ se rechaza $H_{o}$ y se acepta por lo tanto la hipótesis $H_{1}$ lo que implica suficiente evidencia para acusar a la empresa de realizar pagos inferiores a $\$13.200$. 
...
~\\Acorde a los resultados obtenidos por la secretaría de educación y consignados en la tabla, se procede a plantear la hipótesis con la cual se pueda decidir si es acertado ubicar la escuela en la comuna A. 

~\\Procedemos a plantear la hipótesis 
~\\$H_{o} : P_{1} - P_{2} = 0$
~\\$H_{1} :  P_{1} - P_{2} > 0$

~\\La hipótesis nos lleva a tener que evaluar una diferencia de proporciones, con un coeficiente de significancia de 0.05 y para los cuales se tienen los siguientes datos.
~\\  $P_{A} = 8/50 = 0.16$
~\\  $P_{B} = 9/50 = 0.18$
~\\ $n_{1} = 50$
~\\ $n_{2} = 60$
...
~\\ Estadístico de Prueba $Z_{p} = \frac{(P_{A} - P_{B}) - d }{\sqrt{(P_{A}(1-P_{A})/n1) + (P_{B}(1-P_{B})/n2)}}$

~\\$Z_{p} = \frac{(0.16 - 0.18) - 0 }{\sqrt{(0.16(1-0.16)/50) + (0.18(1-0.18)/60)}}$
~\\$Z_{p} = 0.1444$

~\\  La región de rechazo asignando una confianza del 95\% sera RC = {$\left \{ Z_{p} < Z_{\alpha} \right \} \to  0.1444  < 1.645$}

~\\ \begin{figure}[!h]
    \begin{center}
        \includegraphics[width=10cm]{Figuras/punto2.png}
        \caption{Región de rechazo para $H_{o} : P_{A} = P_{1} - P_{2} = 0$ y $H_{1} : P_{1} - P_{2} > 0$}
        \label{fig:Densidad}
    \end{center}
\end{figure}
...
\pagebreak
~\\ \textbf{Conclusión:}
~\\ Con base en los resultados obtenidos en los cuales nuestro valor $Z_{p} = 0.1444$ se encuentra en la región de aceptación, pese a ser un margen de aceptación bastante bajo es adecuada la decisión de la secretaria de educación de ubicar la escuela en la comuna A, es decir aceptamos la hipótesis nula y por lo tanto con una significancia de $5\%$ podemos afirmar que pese a que la diferencia de proporciones de la población es pequeña podemos aceptar la construcción del centro educativo. 

\subsection{Situaci\'{o}n 6}
\subsection{Punto C.}
...
~\\ Tras realizar los cálculos pertinentes, se obtuvo que 3 Kilogramos equivalen a 6 Libras de peso, ahora se procede a plantear la hipótesis.
...
~\\ $ Z_{p} = \frac{5.456 - 6}{1.872/\sqrt{50}} $
~\\ $ Z_{p} = -2.054$

~\\   La región de rechazo asignando una confianza del 95\% sera RC = {$\left \{ Z_{p} < Z_{\alpha} \right \} \to  -2.054 < -1.645$}

~\\ \begin{figure}[!h]
    \begin{center}
        \includegraphics[width=10cm]{Figuras/punto3.png}
        \caption{Región de rechazo para $H_{o} : \mu = 6$ y $H_{1} : \mu < 6$}
        \label{fig:Densidad}
    \end{center}
\end{figure}
...
\pagebreak
~\\ \textbf{Conclusión:}
~\\ Al realizar la prueba de hipótesis y contrastar los resultados se puede observar como con nivel de significancia del $5\%$ es posible rechazar la hipótesis nula $H_{o}$ implicando esto que al nacer lo niños de la población de estudio cuentan con un peso medio inferior a las 6 Libras o los correspondientes 3KG, los que nos lleva la aceptación de la hipótesis alterna 
...
~\\ Planteando las hip\'{o}tesis tenemos:
~\\ $H_{0}$: $p=0.05$
~\\ $H_{1}$: $p>0.05$

tenemos que n=50, por tanto, podr\'{i}amos considerar la muestra como una muestra grande y por ello, podemos aplicar la prueba con la aproximaci\'{o}n a la normal:

~\\ $Z=\frac{\hat{P}-P_{0}}{\sqrt{\frac{P_{0}(1-P_{0})}{n}}}=\frac{0.35-0.05}{\sqrt{\frac{0.05(0.95)}{50}}}=9.7333$

~\\ La regi\'{o}n de rechazo asignando una confianza del $95\%$ sera: $Rc=\left\lbrace Z|Z>1.645 \right\rbrace$

~\\ \begin{figure}[!h]
    \begin{center}
        \includegraphics[width=10cm]{Figuras/punto3b.png}
        \caption{Región de rechazo para $H_{o} : p=0.05$ y $H_{1} : p>0.05$}
        \label{fig:Densidad}
    \end{center}
\end{figure}

~\\ \textbf{Conclusi\'{o}n:}
~\\ Como Z=9.7333 es mucho mayor que 1.645, rechazamos $H_{0}$ y concluimos que con una confianza del $95\%$, el verdadero porcentaje de niños que presentan bajo peso al nacer en esa comunidad es mayor que $5\%$.


\subsection{Situaci\'{o}n 7}
\begin{center}
\begin{tabular}{|c|c|c|c|c|}
\hline 
\rule[-1ex]{0pt}{2.5ex}  Ciudad & Desempleo Febrero & Desempleo Mayo & Diferencia\\ 
\hline 
\rule[-1ex]{0pt}{2.5ex}  Cali & 15\% & 11\% & 0.04\\ 
\hline 
\rule[-1ex]{0pt}{2.5ex}  Medellin & 12\% & 8\% &0.04\\ 
\hline 
\rule[-1ex]{0pt}{2.5ex}  Pereira & 14.5\% & 11.8\% &0.027\\ 
\hline 
\rule[-1ex]{0pt}{2.5ex}  Manizales & 12\% & 12\% &0\\ 
\hline 
\rule[-1ex]{0pt}{2.5ex}  Cúcuta & 13.8\% & 13.4\% &0.004\\ 
\hline 
\rule[-1ex]{0pt}{2.5ex}  Barranquilla & 14.3\% & 14.8\% &-0.005\\ 
\hline 
\rule[-1ex]{0pt}{2.5ex}  Arauca & 13\% & 13\% &0\\ 
\hline 
\rule[-1ex]{0pt}{2.5ex}  Popayán & 14.2\% & 15\% &-0.008\\ 
\hline 
\rule[-1ex]{0pt}{2.5ex}  Ibagué & 12\% & 13.5\% &-0.015\\ 
\hline 
\rule[-1ex]{0pt}{2.5ex}  Neiva & 12\% & 9\% &0.030\\ 
\hline 
\rule[-1ex]{0pt}{2.5ex}  Zipaquira & 11.5\% & 10.5\% &0.010\\ 
\hline 
\rule[-1ex]{0pt}{2.5ex}  Tunja & 12\% & 9\% &0.030\\ 
\hline 
\rule[-1ex]{0pt}{2.5ex}  Valledupar & 13.5\% & 11.2\% &0.023\\ 
\hline 
\rule[-1ex]{0pt}{2.5ex}  Sta. Marta & 14\% & 13\% &0.010\\ 
\hline 
\end{tabular} 
\end{center}

\subsection{Punto A.}
~\\ De acuerdo con la información de desempleo consignada en la tabla anterior, se puede proceder a plantear la hipótesis para verificar si el programa piloto lanzado por El Ministerio de Protección Social tuvo los resultados esperados. 

~\\Procedemos a plantear la hipótesis 
~\\$H_{o} : \mu_{D} = 0.02$
~\\$H_{1} : \mu_{D} > 0.02$

~\\ A partir de una significancia del 0.05, se selecciona y calcula el estadístico de prueba con los siguientes datos:
~\\ $n=14$
~\\ $\bar x = 0.01328 $
~\\ $Sx = 0.01822 $
~\\ $Z_{\alpha} = Z_{0.05} = 1.77$ 

~\\ Estadístico de Prueba $ Z_{p} = \frac{\bar x - \mu}{\sigma/\sqrt{n}}$

~\\ $ Z_{p} = \frac{0.01328  - 0.02}{0.01822/\sqrt{14}} $
~\\ $ Z_{p} = -1.378$

~\\ La región de rechazo para el nivel de significancia asignado sera $\left\{ Z_{p} < Z_{\alpha} \right \} \to  -1.378 < 1.77.$

~\\ \begin{figure}[!h]
    \begin{center}
        \includegraphics[width=10cm]{Figuras/punto4.png}
        \caption{Región de rechazo para $H_{o} : mu_{D} = 0.02$ y $H_{1} : \mu_{D} > 0.02$}
        \label{fig:Densidad}
    \end{center}
\end{figure}
...
\pagebreak
~\\ \textbf{Conclusión:}
~\\ Tras realizar la prueba de hipótesis se puede observar como el valor $ Z_{p}$ es mucho meno al  $Z_{\alpha}$ y es posible contar con la suficiente evidencia para aceptar la hipótesis nula y por lo tanto concluir con una de significancia del 5\% que el nivel de desempleo afronta una disminución superior al 2\% por lo cual El Ministerio de Protección Social puede dar como exitoso el programa tras tres meses de pruebas. 

\subsection{Punto B.}
~\\ Para poder llevar a cabo la evaluación del supuesto requerido es necesario implementar la prueba Shapiro-Wilk, dicha prueba es la encargada de comprobar si la muestra proviene de una población con distribución Normal, esta prueba puede ser aplicada cuando la muestra es de un tamaño máximo de 50 para efectuarla se calcula la media y la varianza muestral.

~\\ El estadístico de prueba para esta dado por: $\frac{D^2}{nS^2}$ donde D es la suma de las diferencias corregidas.
...
~\\Procedemos a plantear la hipótesis 
~\\$H_{o}$ : Los Datos provienen de una Distribución Normal
~\\$H_{1}$ : Los Datos  NO provienen de una Distribución Normal

\pagebreak
~\\ Para realizar el análisis de normalidad (test Shapiro-Wilk) se hace uso del paquete estadístico R con el cual se obtienen los siguientes resultados:

\begin{center}
\begin{tabular}{|c|c|c|c|c|}
\hline 
\rule[-1ex]{0pt}{2.5ex}  Shapiro-Wilk & W & P-Valor\\ 
\hline 
\rule[-1ex]{0pt}{2.5ex}   & 0.93568 & 0.3658\\ 
\hline 
\end{tabular} 
\end{center}
...
~\\ \begin{figure}[!h]
    \begin{center}
        \includegraphics[width=10cm]{Figuras/punto7b.png}
        \caption{Histograma de Frecuencias para la Diferencia y curva de Normalidad}
        \label{fig:Densidad}
    \end{center}
\end{figure}
...
~\\ \begin{figure}[!h]
    \begin{center}
        \includegraphics[width=10cm]{Figuras/punto7b2.png}
        \caption{Curva de normalidad para la Densidad}
        \label{fig:Densidad}
    \end{center}
\end{figure}
...
\pagebreak
~\\ \textbf{Conclusión:}
~\\ Dado que el valor P es mucho mayor al nivel de significancia $0.3685>0.05$ no podemos rechazar que la distribución sea normal, dicha conclusión es mucho mas visible si la apreciamos gráficamente como se muestra en la gráfica 

\pagebreak\section{Regresi\'{o}n}
...
~\\ No tenemos formula para la estimaci\'{o}n por intervalos de la esperanza de Y dado un valor $X=x_{0}$, esto es $E[Y|X=x_{0}]$ por lo cual, trataremos de construirlo, como un intervalo para la media, esto seria:
